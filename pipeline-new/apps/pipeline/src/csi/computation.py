"""
CSI Computation Utilities
Core algorithms for Clean Street Index calculation
"""
import logging
...
def find_xmin_xmax(src: np.array) -> tuple:
    ...
    :param src: Input numpy array (2D)
    :return: Tuple of (x_min, x_max) column indices
    """
    if __debug__:
        if src.ndim != 2:
            msg = f"Invalid ndim for src: {src.ndim}. Must be 2."
            logging.error(msg=msg)
            raise ValueError(msg)
        
        if src.shape[0] < 1:
            msg = f"Invalid number of rows in src: {src.shape[0]}. Must be > 0."
            logging.error(msg=msg)
            raise ValueError(msg)
        
        if src.shape[1] < 1:
            msg = f"Invalid number of cols in src: {src.shape[1]}. Must be > 0."
            logging.error(msg=msg)
            raise ValueError(msg)
...
def get_weight_matrix_linspace(n_rows: int,
    ...
    (in n_bins steps) on the left side, remain constant in the middle, and then 
    symmetrically mirror back from linsp_stop to linsp_start on the right side.
    
    This gives lower weights to garbage near the sidewalk and higher weights to 
    garbage at the center of the road.
    ...
    grouping = n_cols // (n_bins * 2)  # *2 because half for left, half for right
    ...
    # Create a matrix with n_rows, each row composed of repeated columns of weights
