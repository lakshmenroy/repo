...
VERIFIED against pipeline/csi/utils/probes/probe_functions.py
"""
import logging
import sys
import time
...
# Get project root - for monorepo structure
FILE = Path(__file__).resolve()
PROJECT_ROOT = FILE.parents[4]  # truck-ai-stack/
PIPELINE_ROOT = FILE.parents[2]  # apps/pipeline/
...
# Load CSI configuration - VERIFIED path
config_path = PIPELINE_ROOT / "config" / "csi_config.yaml"
    ...
    while l_frame is not None:
        ...
        while l_user is not None:
            ...
            if seg_user_meta and seg_user_meta.base_meta.meta_type == pyds.NVDSINFER_SEGMENTATION_META:
                ...
                # Display masks for debugging (only if debug enabled)
                if csi_config.debug:
